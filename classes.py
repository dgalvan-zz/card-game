...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        """
        "*** YOUR CODE HERE ***"
        discard_cards = opponent.hand[:3]
        opponent.hand = opponent.hand[3:]
        for i in range (3):
            if not opponent.deck.is_empty():
                new = opponent.deck.draw()
                opponent.hand.append(new)
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        opponent_attack = other_card.attack
        opponent_defense = other_card.defense
        
        player_copy = player.deck.cards[:]
        
        for card in player_copy:
            card.attack +=opponent_attack
            card.defense += opponent_defense

        opponent_copy=opponent.deck.cards[:]
        for i, card in enumerate(opponent_copy):
            if card.attack == opponent_attack or card.defense== opponent_defense:
